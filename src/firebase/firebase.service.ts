...
export class FirebaseService implements OnModuleInit {
   ...
   * 채팅방 메시지 목록 조회
   */
  async getMessages(roomId: string, limit: any = 30): Promise<any[]> {
    try {
      // limit 매개변수가 문자열이면 정수로 변환
  ...
  async leaveRoom(roomId: string, userId: string): Promise<boolean> {
    try {
      ...
      if (participantDoc.exists) {
        ...
        // 문서가 없으면 새로 생성
        this.logger.log(`사용자 ${userId}의 참여 기록이 없습니다. 새로운 기록을 생성합니다.`);
        await participantRef.set({
          id: userId,
          isActive: false,
          leftAt: admin.firestore.FieldValue.serverTimestamp(),
          joinedAt: admin.firestore.FieldValue.serverTimestamp() // 동일한 타임스탬프로 설정
        });
      }
